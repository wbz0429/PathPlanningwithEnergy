...
def build():
    ...
    # ---- 11b 为什么封顶:真机功率噪声 ----
    s = numbered(); title(s, "为什么封顶:真机功率含大量不可观测噪声(风/电压/控制)")
    fit(s, os.path.join(EXP, "pub", "仿真vs真机波动.png"), In(0.5), In(2.0), In(7.8), In(4.2))
    tb(s, In(8.5), In(2.1), In(4.5), In(4.6),
       [("模型输出平滑,真机实测剧烈波动", 16, INK, True),
        ("· 真机功率变异系数 ≈0.20(50–950W)", 15, INK, False),
        ("· 运动学只解释 ~40%", 15, INK, False),
        ("· 其余 ~60%=风/电压/控制噪声", 15, RED, True),
        ("· 不在可观测输入里 → 学不到", 15, INK, False),
        ("→ 改追每趟能量 ARE(积分抹平噪声)", 15, DEEP, True)], space=11)
    cite(s, "真机 DJI M100 实测功率(V×I);左=模型预测(平滑),右=真机片段(波动)")
