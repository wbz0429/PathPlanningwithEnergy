# -*- coding: utf-8 -*-
"""build_deck.py — 生成中期答辩可编辑 PPTX(遵循 academic-pptx skill:action titles / 一页一论点 / 图左释右 / 必引用)。
配色:深蓝 title/conclusion + 浅色 content(sandwich);accent 青。CJK 字体 微软雅黑。图 + 飞行 gif 多放。
"""
import os
...
EXP = os.path.join(HERE, "experiments"); PX4 = os.path.join(HERE, "px4_integration")
GIF = os.path.join(EXP, "gifs")

NAVY = RGBColor(0x1E, 0x29, 0x61); DEEP = RGBColor(0x06, 0x5A, 0x82)
TEAL = RGBColor(0x02, 0x80, 0x90); MINT = RGBColor(0x02, 0xC3, 0x9A)
LIGHT = RGBColor(0xF5, 0xF8, 0xFA); WHITE = RGBColor(0xFF, 0xFF, 0xFF)
INK = RGBColor(0x1A, 0x23, 0x33); MUTED = RGBColor(0x6B, 0x7A, 0x8D)
CJK = "微软雅黑"
...
def tb(slide, l, t, w, h, lines, size=16, color=INK, bold=False, align=PP_ALIGN.LEFT,
       anchor=MSO_ANCHOR.TOP, name=CJK, space=4):
    ...
    for i, ln in enumerate(lines):
        ...
        r.font.color.rgb = cl; _font(r)
    return box


def chip(slide, l, t, txt, color=TEAL):
    from pptx.enum.shapes import MSO_SHAPE
    s = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, l, t, In(0.5), In(0.5))
...
def fit(slide, path, l, t, w, h):
    """在 (l,t,w,h) 框内等比放图,居中。"""
    if not os.path.exists(path):
        tb(slide, l, t, w, In(0.4), f"[缺图 {os.path.basename(path)}]", 12, MUTED); return
...
def title(slide, txt, sub=None):
    # 从编号 chip 右侧起排,避免压字
    tb(slide, In(1.25), In(0.38), In(11.6), In(1.0), txt, 26, DEEP, True)
    if sub:
        tb(slide, In(1.25), In(1.32), In(11.6), In(0.4), sub, 15, TEAL, False)


def cite(slide, txt):
    tb(slide, In(0.55), In(7.0), In(12.2), In(0.35), txt, 11, MUTED, False)
...
def content(slide):
    ...
    # 左侧细青竖条 motif(非标题下划线)
    from pptx.enum.shapes import MSO_SHAPE
    bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, In(0.0), In(0.0), In(0.12), SH)
...
def dark(slide):
    ...
    from pptx.enum.shapes import MSO_SHAPE
    bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, In(0.0), In(0.0), In(0.12), SH)
    bar.fill.solid(); bar.fill.fore_color.rgb = MINT; bar.line.fill.background()
...
def build():
    ...
    tb(s, In(0.9), In(4.4), In(11.5), In(1.2),
       [("A Trustworthy LLM-Agent Autoresearch Framework, Instantiated on UAV Energy", 15, RGBColor(0xCA,0xDC,0xFC), False),
        ("Modeling and Energy-Aware Path Planning", 15, RGBColor(0xCA,0xDC,0xFC), False)], space=3)
    tb(s, In(0.9), In(6.2), In(11.5), In(0.6), "硕士学位论文·中期答辩　|　2026", 15, WHITE, False)

    # 2 背景
    s = S(); content(s); chip(s, In(0.55), In(0.4), "1")
    title(s, "大模型自动科研正在爆发,但它的“发现”普遍不可信")
    tb(s, In(6.9), In(2.0), In(6.0), In(4.2),
       [("· FunSearch / AlphaEvolve / Eureka 在数学、代码、RL 上确有新发现", 17, INK, False),
        ("· 但它们结构上都要求“精确可验证评测器”——作者自认局限", 17, INK, False),
        ("· 朴素流程会过度声称:AI Scientist 独立评估——", 17, INK, True),
        ("    42% 实验编码错误失败、57% 手稿含幻觉数值", 16, RGBColor(0xB8,0x3A,0x3A), False),
        ("    把已发表 prior art 误判为“新颖”(关键词匹配)", 16, RGBColor(0xB8,0x3A,0x3A), False)], space=10)
    tb(s, In(0.7), In(2.2), In(5.9), In(3.2),
       [("57%", 96, DEEP, True), ("朴素 AI4S 手稿含错误或幻觉数值", 18, MUTED, False)], anchor=MSO_ANCHOR.MIDDLE, align=PP_ALIGN.CENTER, space=6)
    cite(s, "来源:Beel, Kan & Baumgart, ACM SIGIR Forum 2025(arXiv:2502.14297)")

    # 3 研究问题(dark)
    s = S(); dark(s); chip(s, In(0.55), In(0.4), "2", MINT)
    tb(s, In(0.9), In(1.7), In(11.5), In(0.7), "研究问题", 22, MINT, True)
    tb(s, In(0.9), In(2.7), In(11.5), In(3.0),
       [("在一个真实、含噪、无干净真值的工程域(无人机能耗与规划),", 30, WHITE, True),
        ("可信的自动科研框架能“得到什么”、又该“诚实承认什么”?", 30, WHITE, True)], space=14)
    tb(s, In(0.9), In(5.6), In(11.5), In(0.9),
       "核心主张:贡献不是新算法或新发现(均为 prior art),而是方法学 + 真机数据锚定 + 诚实能力边界。",
       17, RGBColor(0xCA,0xDC,0xFC), False)

    # 4 框架
    s = S(); content(s); chip(s, In(0.55), In(0.4), "3")
    title(s, "我们把“可信性”工程化为五道相互独立的安全机制")
    from pptx.enum.shapes import MSO_SHAPE
    ...
    for i, (h, d) in enumerate(items):
        c = slide_box = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, In(0.7), y, In(0.55), In(0.55))
    ...
    # 5 能耗迭代
    s = S(); content(s); chip(s, In(0.55), In(0.4), "4")
    title(s, "能耗模型自动迭代把留出误差从 6.88% 降到 1.86%——但这不是重点")
    ...
    tb(s, In(8.5), In(2.1), In(4.4), In(4.4),
       [("· 冻结评测器 + LLM 每轮改一次 featurize", 16, INK, False),
        ...
        ("· 但——结构搜索与随机搜索打平", 16, RGBColor(0xB8,0x3A,0x3A), True),
        ("  (见第 8 页,能力边界)", 15, MUTED, False)], space=12)
    cite(s, "冻结评测器 m100_eval.py;数据:真实 DJI M100(Rodrigues 2021,209 航班)")

    # 6 真机代价改变规划(hero + gif)
    s = S(); content(s); chip(s, In(0.55), In(0.4), "5")
    title(s, "真机验证的能耗代价改变规划决策:避开真实存在的爬升能耗")
    ...
    tb(s, In(9.3), In(2.1), In(3.6), In(4.6),
       [("· 距离/教科书BEMT → 翻墙", 16, INK, False),
        ...
        ("· 因果消融:挖爬升项→决策退回翻墙", 15, INK, False),
        ("下方 GIF:城市走廊逐障碍混合决策", 13, MUTED, False)], space=10)
    cite(s, "效应属 prior art(EcoFlight 2025);我们的区别 = 真机验证代价 + 可信评测")

    # 7 动力学 + PX4
    s = S(); content(s); chip(s, In(0.55), In(0.4), "6")
    title(s, "结论过 RotorPy 动力学与 PX4 真飞控固件栈仍成立(省 14.3%)")
    ...
    # 8 prior art
    s = S(); content(s); chip(s, In(0.55), In(0.4), "7")
    title(s, "但这些“效应”经三次文献查新,全部是已发表的 prior art")
    rows = [("能量感知规划改变路径", "EcoFlight (2025)"),
    ...
    for i, (a, b) in enumerate(rows):
        c = RGBColor(0xEE,0xF4,0xF7) if i % 2 == 0 else WHITE
        r = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, In(0.7), y, In(12.0), In(0.72))
        r.fill.solid(); r.fill.fore_color.rgb = c; r.line.color.rgb = RGBColor(0xDD,0xE3,0xE8); r.line.width = Pt(0.5)
        tb(s, In(0.95), y + In(0.13), In(6.6), In(0.5), a, 17, INK, False)
        tb(s, In(7.7), y + In(0.13), In(4.9), In(0.5), b, 15, DEEP, True)
        y += In(0.8)
    tb(s, In(0.7), In(6.5), In(12.0), In(0.5), "→ 我们主动引用、不声称首创。查新门拦住了把“复现”当“发现”。", 16, RGBColor(0xB8,0x3A,0x3A), True)

    # 9 噪声地板
    s = S(); content(s); chip(s, In(0.55), In(0.4), "8")
    title(s, "性能被数据噪声封顶:52 项模型也压不下 6 项——不是方法不足")
    fit(s, os.path.join(EXP, "domain_value.png"), In(0.55), In(2.0), In(8.6), In(4.5))
    tb(s, In(9.4), In(2.1), In(3.5), In(4.6),
       [("· held-out ARE 6 项即触底 1.88%", 16, INK, False),
        ("· 加到 12/20/52 项不降反升", 16, INK, False),
        ("· 4 种方法殊途同归 ~1.9%", 16, MINT, True),
        ("· 运动学只解释 <40% 瞬时功率", 15, INK, False),
        ("“数据封顶”从主张变测量", 16, DEEP, True)], space=12)
    cite(s, "domain_value.py;对标 Tseng 多项式回归(数据集既定最优)")

    # 10 两域显著性
    s = S(); content(s); chip(s, In(0.55), In(0.4), "9")
    title(s, "同框架:有改进空间的域显著胜随机,噪声封顶的域统计打平")
    ...
    tb(s, In(6.9), In(2.1), In(6.0), In(4.8),
       [("能耗域(封顶):", 17, INK, True),
        ...
        ("  loop 显著低于随机分布,z=−2.38", 16, MINT, True),
        ("  0% 随机更好 → 显著胜", 16, INK, False),
        ("→ null 由域性质决定,非框架失败", 17, DEEP, True)], space=12)
    cite(s, "significance_test.py / planner_significance.py(随机搜索分布,z 检验)")

    # 11 用vs不用
    s = S(); content(s); chip(s, In(0.55), In(0.4), "10")
    title(s, "用 vs 不用:朴素 AI4S 声称 4 项假发现且藏负结果,我们相反")
    fit(s, os.path.join(EXP, "naive_vs_trustworthy.png"), In(0.6), In(2.0), In(9.2), In(4.4))
    tb(s, In(10.0), In(2.2), In(2.9), In(4.4),
       [("朴素:", 16, RGBColor(0xB8,0x3A,0x3A), True),
        ("4 假发现 + 0 负结果", 15, INK, False),
        ("我们:", 16, MINT, True),
        ("0 真新 + 4 负结果", 15, INK, False),
        ("一个自信但错,", 15, INK, False),
        ("一个 humbler 但对。", 15, INK, True)], space=12)
    cite(s, "naive_vs_trustworthy.py(同数据同候选,端到端产出对照)")

    # 12 诚实边界 + n=250
    s = S(); content(s); chip(s, In(0.55), In(0.4), "11")
    title(s, "诚实边界:多数场景无收益,载荷无效,贡献是方法学而非发现")
    ...
    tb(s, In(8.4), In(2.1), In(4.5), In(4.6),
       [("· n=250 场景:中位省能 0%", 16, INK, False),
        ...
        ("· 载荷 ≤500g:全扫不改变路径(死路)", 16, INK, False),
        ("· 无真机飞行(止于仿真)", 16, INK, False),
        ("→ 不挑场景、不硬凑,诚实统计", 16, DEEP, True)], space=11)
    cite(s, "large_scale_savings.py(250 随机城市场景);可信 null 是被接受的贡献(ICML 2024)")

    # 13 结论(dark)
    s = S(); dark(s); chip(s, In(0.55), In(0.4), "12", MINT)
    tb(s, In(0.9), In(1.4), In(11.5), In(0.7), "结论", 24, MINT, True)
    tb(s, In(0.9), In(2.4), In(11.7), In(4.4),
       [("① 不声称新算法/新效应——三次查新确认均为 prior art,主动引用", 21, WHITE, False),
        ("② 贡献 = 可信自动科研方法学 + 真机数据锚定 + 诚实能力边界", 21, MINT, True),
        ("③ 逐个消融证明每道安全机制“承重”:去掉即 overclaim", 21, WHITE, False),
        ("④ 价值 = 在该 overclaim 处不 overclaim,并量化能力边界", 21, WHITE, False),
        ("⑤ 域相关:有 headroom 显著胜随机、封顶域正确产出 null", 21, WHITE, False)], space=16)

    # 14 参考文献
    s = S(); content(s); chip(s, In(0.55), In(0.4), "13")
    title(s, "参考文献")
    ...
    for name, fig_path, cap in [
        ("附录 A:操作包络相图(能量感知何时有用)", os.path.join(EXP, "phase_diagram.png"), "高速+窄障省 22%,低速/宽障归零"),
        ("附录 B:安全机制消融(评测器抗 gaming)", os.path.join(EXP, "safeguard_ablation.png"), "过拟合探针刷不动;查新门降级 2/3"),
        ("附录 C:动力学实飞功率剖面", os.path.join(EXP, "sim_flight.png"), "翻墙爬升段飙 870W,绕行平稳"),
        ("附录 D:PX4 真飞控轨迹(俯视/侧视)", os.path.join(PX4, "px4_traj.png"), "翻矮楼 A + 绕高楼 B")]:
        s = S(); content(s)
        title(s, name)
        fit(s, fig_path, In(0.8), In(1.9), In(11.7), In(4.9))
        cite(s, cap)

    out = os.path.join(EXP, "midterm_defense.pptx")
    prs.save(out)
    print(f"生成 {out}({len(prs.slides.__iter__.__self__._sldIdLst)} 页)")
