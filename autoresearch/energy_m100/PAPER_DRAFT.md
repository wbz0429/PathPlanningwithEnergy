...
**Honest boundary.** A synthetic-probe also confirms what the diagnostic does *not* do: with noise diluting a true interaction, single-term statistics alone cannot always separate high from low synergy pools (`headroom_mechanism_probe.json`). The diagnostic is a reliable **saturation detector** (correct on 2 real domains + 1 controlled synthetic + fine-grid); "does an escapable program space exist" is a separate question answered by inspecting the mutation space, not the feature pool.
