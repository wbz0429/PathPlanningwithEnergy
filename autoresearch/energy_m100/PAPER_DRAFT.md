...
> **Status: full first draft.** All numbers come from the frozen evaluator `m100_eval.py` and committed experiment JSONs in `experiments/`. Scripts are committed; every figure regenerates via `pubfigs.py` + the experiment scripts.

---

## Abstract

Large language model (LLM) agents are increasingly proposed for autonomous scientific discovery (FunSearch, AlphaEvolve, Eureka, AI Scientist). Their flagship successes, however, assume **exact, verifiable evaluators** — a condition that fails in most engineering domains, where data is noisy, hardware is idiosyncratic, and no clean ground truth exists. Independent evaluation of the AI Scientist pipeline documents the failure mode: 42% of experiments failed on coding errors, 57% of manuscripts contained hallucinated numbers, and published prior art was misjudged as novel. We present a **trustworthy autoresearch protocol** for such domains, instantiated on real DJI M100 UAV energy modeling and energy-aware path planning, and contribute a **pre-loop headroom diagnostic** that predicts — before any search budget is spent — whether guided search will beat random. The protocol has four stages: (1) a **prior-art knowledge base** of 14 published UAV energy models; (2) a **zero-fit transfer benchmark** showing all as-published models fail on the target platform (energy ARE 53–76%), motivating — not assuming — a refit stage; (3) a **cheat-proof discovery loop** (frozen held-out evaluator, keep/revert on held-out, forced novelty gate, causal ablation, honest negative reporting) starting from the refit best; (4) **quantified capability boundaries**. The headroom diagnostic correctly predicts that the energy domain's search space is saturated (tie at all 9 controlled complexity levels, matching observed advantage ≈ 0), while the planning domain's win (z = −2.38, 0% of random runs beat it) is entirely attributable to cross-space escape into program space. Every safeguard is shown load-bearing by ablation. The protocol converges to human-expert-level modeling accuracy (loop product 1.93% vs. the same-dataset human result 1.90%) while honestly exposing its own limits.
