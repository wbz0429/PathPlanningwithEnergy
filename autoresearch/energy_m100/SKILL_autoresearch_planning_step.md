...
2. **取假设**:队列首个 open 假设;若空,自提一个(格式:"在___场景/任务下,能量感知相比距离基线会___,
   ...
   **轻文献检查(每轮必做,2-4 条 WebSearch)**:这个假设有没有人做过?已发表的幅度是多少?
   把带来源的对标数字记进 PLANNING_KNOWLEDGE「文献对标」节(我们的结果要和文献幅度对得上量级,差太远=先怀疑自己)。
   **硬门:本轮「文献对标」没有新增带来源的条目 → 不许出裁决**(M-pl-1 期间此步被跳过,已列为协议违规教训)。
...
仅以下三种情况调用 `Workflow(name="deep-research", args="<问题纯字符串>")`(⚠️ args 必须是字符串,传对象会报 No research question):
