...
<p><b>核心节点:外搜如何逃出局部最优</b>。只在"物理形式"里局部搜索会困在 ~4.2% 的盆地——七个物理变体
(T^1.4/T^1.6/Glauert 桥/轴向 sqrt/v^2.5/T·v²)全部卡在 4.24–4.48%;<b>外搜(文献检索指出缺口是线性 payload 而非更花哨的物理)
识别出逃逸方向,一步突破到 1.93%</b>。这直接证明"外搜逃局部最优"这一节点承重。</p>
{fig("framework_ablation.png", "7b", "局部最优 × 外搜消融:七个物理形式变体困在 ~4.2% 盆地(红),外搜识别'加线性 payload'一步突破 ~1.9%(绿)")}
<p><b>累加消融阶梯</b>:从朴素 loop(无安全机制)开始逐步加回每道机制,同时看两个正交指标——
会声称的假发现数、报告的留出误差。朴素配置若报拟合误差会<b>虚低到 1.99%</b>并声称 <b>3 项假发现</b>;
加冻结留出后报告数字变诚实(1.93%),加强制查新门后<b>假发现归零</b>,完整框架维持诚实且零假发现。</p>
{fig("ablation_ladder.png", "7c", "累加消融阶梯:逐步加回安全机制,假发现 3→0(左),报告的留出误差保持诚实、不虚低(右)")}
<p><b>框架各节点消融总表</b>(每个节点 → 去掉它会怎样 → 证据):</p>
<table><tr><th>框架节点</th><th>去掉它 / 替代</th><th>会怎样(实测)</th></tr>
<tr><td>① loop 本身</td><td>换成随机搜索</td><td>能耗域打平(z=−0.35)、规划器域输(z=−2.38)——价值域相关</td></tr>
<tr><td>② 准确评估(冻结留出评测器)</td><td>改为只报训练误差 / 逐样本 R²</td><td>过拟合探针可刷分;留出戳穿(诚实模型 train 1.83% vs 留出 2.15%)</td></tr>
<tr><td>③ 局部最优逃逸(外搜/文献检索)</td><td>只在物理形式里局部搜索</td><td>困在 ~4.2% 盆地(七变体全卡);外搜指出加 payload 才突破 1.9%</td></tr>
<tr><td>④ 查新门(外搜判新)</td><td>不做文献查新</td><td>把 H1/H3 当发现上报(实为 Michel 2024 / Nguyen 2017)</td></tr>
<tr><td>⑤ 因果消融</td><td>只看相关、不做消融</td><td>无法证明爬升是绕行唯一因(挖掉→决策退回翻墙)</td></tr>
<tr><td>⑥ 交叉模型破循环</td><td>只用自己的模型评</td><td>循环:M100 路只在 M100 尺下省,BEMT 尺下反贵</td></tr>
<tr><td>⑦ 诚实报负结果</td><td>只报成功</td><td>漏报 loop≈随机 / 载荷死路 / 77% 场景 0 收益</td></tr>
</table>
...
{fig("large_scale_savings.png", "11", f"省能分布(n={lss.get('n','250')} 随机城市场景):多数场景为 0,少数几何下 3–15%——诚实的统计陈述,非挑选个例")}
