...
concepts as novel. Falsification-first framing is prior art \citep{aigs}; our
contribution is the engineered protocol plus empirical boundaries. This gap is
unoccupied.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{模型知识库排行榜.png}
...
$\pm5\%$. The safeguard ablations (\texttt{safeguard\_ablation.py}) show every mechanism
is load-bearing.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{头部空间诊断_三域验证.png}
...
\label{sec:threats}
\paragraph{T1: ``An LSTM beats your energy model by 72\% RMSE (Muli et al.).''}
...
vs.\ 1.90\% per-flight energy ARE; the LSTM's 36 RMSE is per-sample, a different quantity
and a black box unusable as an interpretable planning cost).
