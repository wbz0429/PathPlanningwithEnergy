...
S("<h2><span class='n'>11b</span>为什么封顶:真机功率剧烈波动,当前输入只能解释约四成</h2>" +
  two_col(img(pub="仿真vs真机波动.png"),
          bl(["<b>模型输出平滑,真机实测剧烈波动</b>",
              "真机功率变异系数 ≈0.20(50–950W 乱跳)",
              "运动学(速度/爬升/载荷)只解释 ~40%",
              "其余 ~60% <b>无法由当前输入解释</b>(具体来源待归因)",
              "<b class='blu'>→ 剩余部分当前学不到,构成误差下界</b>",
              "所以不追逐点 R²,改追每趟能量 ARE(积分抹平波动)"])) +
  "<div class='cite'>真机 DJI M100 实测功率(V×I);左图为模型预测(平滑),右图为真机片段(波动)</div>")
