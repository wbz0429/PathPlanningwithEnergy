...
# ① 数据集 + 问题 + 别人做到哪(导师要求)
S("<h2>研究背景:真实数据集 + 为什么有这个问题 + 已有工作</h2>" +
  "<table class='fw'>" +
  "".join(f"<tr><td class='c1'>{a}</td><td class='c3'>{b}</td></tr>" for a, b in [
      ("数据集", "<b>DJI Matrice 100,209 次真实飞行</b>(Rodrigues 2021, CC-BY):机载电压×电流真功率,载荷 0/250/500g,海拔 25–100m,速度 4–12 m/s"),
      ("为什么有这个问题", "无人机能耗建模与能量感知规划均依赖准确的能耗代价;而<b>可信的自动科研</b>(LLM 驱动)在此真实含噪域的可信度未知——会不会过拟合、会不会把已知当发现"),
      ("已有能耗建模", "Tseng 等(数据集论文)既定最优=9 项多项式回归(留出 ARE ~1.9%);Dai 已做过 M100 建模;<b>经典 BEMT 为教科书基线(6.88%)</b>"),
      ("已有能量感知规划", "EcoFlight(2025)、Michel(2024,配送顺序)、Nguyen&Au(2017,可达集)等已研究;本文<b>主动引用,不声称首创</b>"),
      ("本文的区别", "<b>用真机数据验证的可信代价</b>(vs 教科书理论模型),并以防作弊闭环做自动科研——贡献在方法学与真机锚定,非新效应")]) + "</table>" +
  "<div class='cite'>Rodrigues et al. Scientific Data 2021;Tseng(arXiv:2206.01609);Michel 2024;Nguyen & Au 2017;EcoFlight 2025</div>")
...
# ③ 几个模型对比(BEMT vs 我们的 vs 随机模型,具体形式)
S("<h2>能耗模型对比:教科书 BEMT vs 我们的模型 vs 随机搜索最优</h2>" +
  "<table class='fw'><tr class='hd'><td>模型</td><td>形式</td><td>留出 ARE</td><td>特点</td></tr>" +
  "".join(f"<tr><td class='c1'>{a}</td><td>{b}</td><td class='c2'>{c}</td><td>{d}</td></tr>" for a, b, c, d in [
      ("教科书 BEMT", "稳态物理公式:仅 1, v, v²", "6.88%", "理论推导,无真实数据验证"),
      ("我们的模型(loop)", "动量核 T^1.5 + T²/V + v³ + 爬升/下降不对称 + 载荷 payload", "<b>1.86%</b>", "物理结构 + 真机数据拟合,含爬升项→指导规划"),
      ("随机搜索最优(iter8)", "纯多项式:v, v², v³, payload, v·pay, v²·pay, climb+, |az|, ah, omega", "1.91%", "无物理骨架,纯数据多项式,与我们几乎打平")]) + "</table>" +
...
# ② 爬升耗能对绕行 + 急剧vs缓慢爬升(真机实测)
S("<h2>为什么绕行更省电:爬升很耗能,且急剧爬升远比缓慢爬升贵(真机实测)</h2>" +
  two_col(img(pub="爬升耗能真机实测.png"),
          bl(["<b>真机实测(209 航班,直接量出,非模型猜):</b>",
              "下降 ≈ 平飞(499W vs 494W,几乎一样)",
              "缓慢爬升(1-2 m/s):<b class='blu'>+24W</b>",
              "中爬升(2-4 m/s):<b class='blu'>+104W</b>",
              "急剧爬升(>4 m/s):<b class='red'>+142W</b>",
              "<b>→ 爬升/下降不对称:爬升贵、下降贱</b>",
              "这就是绕行(低空不爬升)比翻越(爬高墙)省电的根因——宁可绕远也不爬"])) +
  "<div class='cite'>真机 DJI M100 实测功率(V×I),按爬升率分箱取均值。急剧爬升比平飞贵 +142W;下降仅 +6W≈平飞</div>")
