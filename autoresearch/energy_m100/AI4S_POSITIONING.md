...
## ★ 决定性论点(2026-07 deep-research):引导搜索的优势随搜索空间复杂度增长
**用户猜想"能耗域打平是因为空间太简单"——文献完全证实,且有精确边界:**
- **Bergstra & Bengio 2012**(JMLR):随机搜索在名义 32 维 HPO 空间追平网格,是因为**有效维度低**(少数超参重要);随机被定位为**baseline 而非天花板**。
- **Wang 2013 REMBO** + **Bayesian 高维**:标准贝叶斯优化在 **~15–20 维临界点**以上退化到仅追平随机;只有利用结构的方法能幸存。(ar5iv 1301.1942)
- **NAS "随机是强baseline"(Li&Talwalkar 2019 等)**:作者明确归因于**过度受限的专家设计cell空间**(随便一个架构都不错),不是引导搜索没用;**去掉混淆(权重共享)后,引导采样大幅胜随机——ENAS 超越随机的概率 0.07→0.90**。(1902.08142)
- **FunSearch/AlphaEvolve**:程序空间大到(cap-set 可能性超过宇宙原子数)无引导枚举不可行,引导搜索成功、随机不可能。

**对我们:能耗模型搜索空间只有十几个线性项(小/低有效维)→ 随机追平是规律预测的结果,不是框架失败。**
**框架的价值在复杂空间——我们规划器域(代码空间,大)已实证 loop 胜随机 27%,正是这条规律的两个端点。**
→ 论文论点:**"我们同一框架横跨两个复杂度端点,亲手复现了这条已知规律:简单空间打平(能耗)、复杂空间显著胜(规划器)。"** 这是正面贡献,不是负结果。
