...
- **[S4a, iter61] 静态采样器空间耗尽判决 + goal-cone REVERT**:goal-approach cone(目标锚定进近走廊,通用无障碍知识)train=806.3 但 heldout(3,4)=1282.6/(5,6,7)=836.9——**第4次连续过拟合**。S4a 尝试账:iter58 比例调优(heldout 1689)、iter59 计划调优(不稳)、iter60 子水平集(1107 更差)、iter61 goal-cone(heldout 1283)——**全部无法鲁棒超过 v1 的 810**。判决:**度量(B/C 首解质量)高度种子脆弱,v1 简洁各向异性椭球是唯一鲁棒点;静态采样器空间在 ctx 限制下(无树/无障碍访问、oracle 平面各向同性)已耗尽**。S4a 核心结果已巩固:v1 −43%(vs 均匀 1419)、跨 3 种子集稳(810/804/801)、消融证明纵向平面聚集是杠杆、超越均匀任意预算(1216 地板)、优于文献子水平集法(1107)。**下一步需人类**:加 program.md 承诺的 SOTA 门(Euclidean-informed)完成开放问题正式对比,或推进 S4b。
- **[S4a, iter60, Ep13 EXPLORE — 归因订正 + 文献法]** 消融分解订正 iter57-58 的过度归因:symmetric(1.0,1.0)=1311→**squash-only(r_vert=0.30)=816(拿走几乎全部增益)**→stretch-only(r_lat=1.7)=1128(只走一半)→v1(both)=810。**真正杠杆是纵向压扁(把样本聚到起终点高度平面),不是横向拉伸**;横向拉伸只贡献最后 6J。**订正框架**:增益是"能量各向异性**动机**的平面聚集",纵向压扁被能量各向异性motivate,但比逐点能量度量justify的更激进(见下)。
- **[S4a, iter60] 文献法(HRS/sub-level-set)反而更差,揭示订正**:攻 cited 论文(Kyaw&Kelly 2026 arXiv:2606.02879 Direct Informed Sampling on Riemannian Manifolds;综述 arXiv:2410.19414,置信度高,未验证-以评测器为准)——非欧代价下 informed set 非椭球,SOTA 用**子水平集拒绝采样/MCMC**:{x: edge_cost(s,x)+edge_cost(x,g)≤c_thresh}。我实现了纯 oracle 子水平集拒绝采样器(自动产生纵向压扁,无需手调轴,可泛化到非平面场景)。**结果=1107(train)/1560(heldout),远差于 v1 810 与几何 squash 816**。诊断:z-clamp 带内(±6m/72m 水平)高度偏移是**缓坡**,edge_cost 几乎不罚 → 能量子水平集**压不够扁**;手调 r_vert=0.30 编码了更强的"最优即平面"先验(比逐点能量度量更对)。**结论**:对平面-最优查询,informed set 需要比原始能量子水平集更强的平面聚集——这既证明能量各向异性方向正确,也揭示纯度量法不足(需领域先验)。REVERT,保 v1;记为 exploration seed(纯度量法在非平面场景才会真正胜出)。
- **[S4a, iter59] 度量本质 + 采样效率头条(诚实重述)**:convergence_eval 里 RRT* **首次连通 goal 即 break**(无 anytime 精修)→ 指标本质 = **固定样本预算下首解质量**,不是渐进收敛。证据:均匀 sampler 在 budget 1500/3000/6000 **恒为 1216(预算无关)**——它反复找到锯齿首解、更多样本也不改善。**头条:v1@1500=810 < 均匀@6000=1216**,即各向异性 informed 找到均匀 4× 预算都找不到的**质变更优解**(不只是"更快")。v1 连通仅 2s vs 均匀 22s。**报告须如实**:写"informed 采样使首解平滑+快速连通",别overclaim"anytime 收敛"。
- **[S4a, iter59] 参数鲁棒最优二次确认**:anneal 计划扫描(p_unif 斜率 / margin 斜率)——v1(p=0.38,m=0.65)训练 810、留出 804.6/801.5 **三种子集全稳**;所有扰动要么更差要么高方差(p=0.25→训练 1843;p=0.38/m=0.45→1066/1455/810)。目标(首解质量)是**脆弱/不连续**的,v1 恰在稀有稳定点。**两次独立扫描(比例 iter58 + 计划 iter59)共同确认 v1 参数是鲁棒最优,扰动即过拟合/失稳** → 参数优化已收敛。
