def sample(ctx):
    ...
    r_lat = 1.7 * base       # stretch: lateral detours are energy-cheap
    r_vert = 0.30 * base     # squash: vertical excursions are energy-expensive
    ...
    # low-dispersion point in unit 3-ball via R3 additive-recurrence
    # (plastic constant), indexed by ctx.iteration -> even, less-clumpy
    # coverage of the informed set => lower seed variance (Janson et al.,
    # deterministic sampling accelerates RRT convergence). A tiny per-seed
    # jitter keeps trees from being identical across seeds.
    n_idx = it + 1.0
    A1 = 0.8191725133961645    # 1/g,  g^4 = g+1 (plastic-like 3D)
    A2 = 0.6710436067037893    # 1/g^2
    A3 = 0.5497004779019702    # 1/g^3
    jit = float(rng.random()) * 0.013
    v0 = (0.5 + n_idx * A1 + jit) % 1.0
    v1 = (0.5 + n_idx * A2 + jit) % 1.0
    v2 = (0.5 + n_idx * A3 + jit) % 1.0
    rad = v0 ** (1.0 / 3.0)
    ct = 2.0 * v1 - 1.0
    st = np.sqrt(max(0.0, 1.0 - ct * ct))
    phi = 2.0 * np.pi * v2
    u = np.array([rad * st * np.cos(phi), rad * st * np.sin(phi), rad * ct])
