def sample(ctx):
    ...
    if r < 0.08:
        return g.copy()
    # (1b) goal-approach cone: a runway into the goal so the first RRT*
    #      goal-connection arrives along an aligned (smooth) final approach
    #      rather than a random angle. General: uses only start/goal.
    if r < 0.08 + 0.15:
        d = g - s
        ...
        if Lg > 1e-6:
            ...
            perp = np.cross(ug, ez0)
            npd = float(np.linalg.norm(perp))
            if npd < 1e-6:
                perp = np.cross(ug, np.array([1.0, 0.0, 0.0])); npd = float(np.linalg.norm(perp))
            perp = perp / max(npd, 1e-9)
            back = float(rng.uniform(2.0, 22.0))          # distance back from goal
            spread = 0.18 * back                            # cone half-width grows with distance
            off = float(rng.normal()) * spread
            zoff = float(rng.normal()) * (0.25 * spread)    # keep near-plane (energy)
            return g - ug * back + perp * off + ez0 * zoff
