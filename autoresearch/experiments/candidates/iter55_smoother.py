...
def smooth_path(path, is_collision_free, config):
    ...
    # ---- final deterministic polish: harvest the stochastic tail ----
    # STOMP is the last stochastic stage; its output was never re-polished.
    # One deterministic sweeps->CHOMP round harvests what its noise found
    # and corrects what its noise broke (reduces per-seed variance).
    cand = _sweeps([p.copy() for p in pts])
    cand = _chomp(cand)
    if _proxy3d(cand) < _proxy3d(pts) - 1e-6:
        ok = True
