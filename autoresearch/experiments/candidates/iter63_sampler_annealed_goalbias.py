def sample(ctx):
    ...
    r = float(rng.random())
    # (1) ANNEALED goal bias: low early so the first RRT* goal-connection is
    #     delayed until the tree has matured through the corridor (RRT* breaks
    #     on first connection, so first-path quality = tree maturity at connect);
    #     ramps up late to force connection. Ellipsoid still steers growth
    #     toward goal, so low early bias doesn't starve reachability.
    gb = 0.02 + 0.30 * frac
    if r < gb:
        return g.copy()
